...
improvements, the noise of overview \fxnote{Ida: overview->summary?} articles and the effect of 
...
\subsection{Testing the Sosine Similarity Measure\label{TestingCosineSimilarity}}
...
with the two remaining scores from here on. Note that the score that \fxnote{Ida: hvad betyder ``Note that the score that''?}
...
would no sense, as we will be running on non-normalized data. Or in
...
We have also chosen to cluster the disease where the sum measure on the
...
its suggestion of this disease, and might provide clues to correcting
...
\noindent \small{\textit{*Note that the scores below are 0-indexed as in the previous sections.}}
\begin{figure}[H]
