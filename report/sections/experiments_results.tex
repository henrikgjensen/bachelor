...
improvements, the noise of overview atricles and the effect of 
...
and the queries made are based on symptoms extracted from the cases
...
The tests are shown in the figures below \ref{termDoc_bmj_hist_3000_ns_mea_ns_mea_sqr_s_max_s_max_sqr} and \ref{termDoc_orphan_hist_3000_ns_mea_ns_mea_sqr_s_max_s_max_sqr}, where we compare the best measures from above 
...
makes no sense since we will be running on non-normalized data. Or in
...
list. This means that these diseases are closest to each other in the list
...
\textit{Kikuchi disease} which is 6 sub clusters away from \textit{Cat
  Scratch disease} - it requires merging 6 sub clusters
...
We have also chosen to cluster the diseases where the sum measure on the
...
its suggestion of this disease and it might provide clues to correcting
it. The problem might stem from poor or very noisy information about
...
\subsection{Results and analysis of the blind tests (provided by Henrik L. J\o rgensen)\label{Blindtest}}

Below are shown the results from from the blind tests described in figure \ref{blind_test_table} -
note that the scores above are 0-indexed as in the previous sections. 
...
term document matrix, using the cosine measures, is seen in table \ref{ThisIsATable}.
